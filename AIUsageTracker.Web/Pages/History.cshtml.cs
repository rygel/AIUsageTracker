...
public class HistoryModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetExportAsync()
    {
        ...
        {
            var quota = usage as QuotaProviderUsage;
            ...
            csv.Append((quota?.RequestsUsed ?? 0).ToString(CultureInfo.InvariantCulture)).Append(',');
            csv.Append((quota?.RequestsAvailable ?? 0).ToString(CultureInfo.InvariantCulture)).Append(',');
            csv.Append((quota?.UsedPercent ?? 0).ToString(CultureInfo.InvariantCulture)).Append(',');
