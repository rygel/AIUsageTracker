...
namespace AIUsageTracker.UI.Slim
{
    ...
    public partial class MainWindow : Window
    {
            ...
            UiPreferencesStore? preferencesStore)
        {
            ...
            {
                try { await this.SaveWindowPositionAsync(); }
                catch (Exception ex) { this._logger.LogError(ex, "LocationChanged handler failed"); }
            };
            SizeChanged += async (s, e) =>
            {
                try { await this.SaveWindowPositionAsync(); }
                catch (Exception ex) { this._logger.LogError(ex, "SizeChanged handler failed"); }
            };
            Activated += (s, e) =>
            {
