...
namespace AIUsageTracker.Infrastructure.Providers
{
    ...
    public class OpenAIProvider : ProviderBase
    {
        ...
        public override async Task<IEnumerable<ProviderUsage>> GetUsageAsync(ProviderConfig config, Action<ProviderUsage>? progressCallback = null)
        {
            ...
            {
                return await this.GetApiKeyUsageAsync(config.ApiKey);
            }
            ...
            {
                var nativeAuth = await this.LoadOpenCodeAuthAsync();
            ...
            try
            {
                return new[] { await this.GetNativeUsageAsync(accessToken, accountId) };
            }
            catch (Exception ex)
            {
