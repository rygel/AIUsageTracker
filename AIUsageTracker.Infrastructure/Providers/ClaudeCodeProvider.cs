    ...
    private async Task<ProviderUsage?> GetUsageFromApiAsync(string apiKey)
    {
        try
        {
            ...
            if (rateLimitHeaders.RequestsLimit > 0)
            {
                ...
                tooltipDetails.Add(new ProviderUsageDetail { Name = "Current RPM Usage", Used = $"{usagePercentage:F1}%", DetailType = ProviderUsageDetailType.Other, QuotaBucketKind = WindowKind.None });
