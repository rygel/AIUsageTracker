    ...
    public class GitHubCopilotProvider : ProviderBase
    {
        ...
        public override async Task<IEnumerable<ProviderUsage>> GetUsageAsync(ProviderConfig config, Action<ProviderUsage>? progressCallback = null)
        {
            ...
            try
            {
                ...
                {
                    await this.PopulateProfileAndCopilotDataAsync(token, response, state);
                }

                await this.PopulateUsernameFallbackAsync(state);
        ...
        private async Task PopulateProfileAndCopilotDataAsync(string token, HttpResponseMessage response, CopilotUsageState state)
        {
            ...
            await this.PopulatePlanNameAsync(token, state);
            await this.PopulateQuotaSnapshotAsync(token, state);
        }
        ...
        private async Task PopulateQuotaSnapshotAsync(string token, CopilotUsageState state)
        {
            try
            {
                ...
                using var quotaResponse = await this._httpClient.SendAsync(quotaRequest);
