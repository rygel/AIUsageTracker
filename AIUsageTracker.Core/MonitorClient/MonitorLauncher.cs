...
public class MonitorLauncher
{
    ...
    public static async Task<bool> StartAgentAsync()
    {
        ...
        try
        {
            ...
            var port = await GetAgentPortAsync().ConfigureAwait(false);
            var monitorExeName = GetMonitorExecutableName();

            var agentPath = FindMonitorExecutable(monitorExeName);
            if (agentPath != null)
            {
                return LaunchMonitorExecutable(agentPath, port);
            }

            return TryLaunchViaDotnetRun(port);
        }
        catch (Exception ex)
        {
    ...
    private static string GetMonitorExecutableName()
    {
    ...
    private static string? FindMonitorExecutable(string monitorExeName)
    {
        var possiblePaths = GetPossibleMonitorPaths(monitorExeName);
        MonitorService.LogDiagnostic($"Locating Monitor executable (checked {possiblePaths.Length} common locations)...");
        return possiblePaths.FirstOrDefault(File.Exists);
    }

    private static string[] GetPossibleMonitorPaths(string monitorExeName)
    {
        return new[]
        {
            // Development paths
    ...
    private static bool TryLaunchViaDotnetRun(int port)
    {
        ...
        LaunchMonitorViaDotnet(agentProjectDir, port);
        return true;
    }

    private static void LaunchMonitorViaDotnet(string agentProjectDir, int port)
    {
    ...
    private static bool LaunchMonitorExecutable(string agentPath, int port)
    {
