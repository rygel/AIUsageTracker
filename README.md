...
![Version](https://img.shields.io/badge/version-2.3.2--beta.5-orange)
...
- [Monitor Guidelines](docs/AGENTS.md)
...
- **Display Mode Toggle**: Switch between **Remaining** and **Used** percentage presentation on provider cards.
- **Pace-Aware Quota Colours**: Optional pace adjustment keeps rolling-window providers green when usage is under expected elapsed pace and shows an **On pace** badge.
- **Usage Rate Badge**: Optional **req/hr** badge for live burn-rate visibility on provider cards.
...
### Manual
1. Download the latest `AIUsageTracker_Setup_v*.exe` from the [Releases](https://github.com/rygel/AIUsageTracker/releases) page.
2. Run the installer.
...
- **Show Used Percentages**:
    - **Checked**: Cards display **Used** percentages.
    - **Unchecked (default)**: Cards display **Remaining** percentages for quota-style providers.
- **Pace-Aware Quota Colours**:
    - **Checked (default)**: Rolling-window providers use time-adjusted pace for color/alert evaluation and can show an **On pace** badge.
    - **Unchecked**: Colors and alerts use raw used percentage only (no pace adjustment).
